...
| **M6 · 生产级改造** | demo → 生产级：包边界收口、审查整改（P0/P1/P2）、CI/质量门、可观测与安全、发布工程、分发矩阵、治理文档 | `packages/{types,memory,artifact,sandbox,policy,core,tools-basic,mock,host,mcp,provider-openai,store-sqlite}` | 🟡 **Gate 1~4 已关闭，P5/P6 收尾中（2026-09-09）**：Gate 1（2026-09-07）→ Gate 2（P2 质量门，M6-14~17）→ Gate 3（P3 可观测与安全，M6-18~M6-21，含 P3 评审 8 项发现全部修复，见 `docs/p3-review.md`）→ Gate 4（P4 SDK 发布工程，M6-22：MIT LICENSE、12 包去 `private` 与发布元数据、`engines >=22.13` + `.nvmrc`/`packageManager`、changesets 编排、`core`/`types` peer 边界、包体积基线）；**P5 分发部署矩阵**：P5.5 生产存储基线（schema 版本化/索引/在线备份）与 P5.6 Web 部署样例（`deploy/` + `/healthz` 探活）已完成（M6-24），P5.1~P5.4 桌面形态补齐 sidecar 构建、脚本化验收与三平台 CI（M6-25），实机签名验收待 Developer ID 证书；**P6 治理文档**：P6.1/P6.2 已完成（M6-23）。**C8 host 为破坏性变更**（`SessionManager` 改从 `@agent-runtime/host` 导入）。逐项状态见 `docs/m6-productionization.md` |
| **M7 · 底座收敛与治理交付** | 以 **12 包为唯一一等公民**：成本与上下文治理、可观测与合规导出、策略工程化、工具规模治理；`examples/` 与 `deploy/` 降级为验证载体，桌面端移出至产品侧 | `packages/{types,core,policy,mcp,provider-openai}` 增量 + 文档口径收敛 | ☐ **未启动（口径已定，2026-09-10）**：立项口径与边界见 `docs/base-convergence.md`（只对底座立项）；候选与首批建议见 `docs/product-direction.md` §5（首批 M7-1/2/3/6）；P5.1~P5.4 已随桌面端移出至产品侧（不再是底座保留项），不进关键路径 |
