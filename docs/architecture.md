...
| **M7 · 底座收敛与治理交付** | 以 **12 包为唯一一等公民**：成本与上下文治理、可观测与合规导出、策略工程化、工具规模治理；`examples/` 与 `deploy/` 降级为验证载体，桌面端移出至产品侧 | `packages/{types,core,policy,mcp,provider-openai}` 增量 + 文档口径收敛 | 🟡 **进行中（M7-2 横切面已落地，2026-09-11）**：立项口径与边界见 `docs/base-convergence.md`（只对底座立项）；候选与首批建议见 `docs/product-direction.md` §5（首批 M7-1/2/3/6a）；**执行清单见 `docs/m7-base-governance.md`**（2026-09-10 立：各包落点 / API 变更分级 / 验收用例 / 顺序）；**已交付**：批次 A（版本 bump 到 0.3.0，未发布、未打 tag）与 **M7-2 的 traceId 贯穿**（19 个事件统一 `traceId?` + `emit()` 与 `redact()` 同点收口 + `Logger.child` + `run:start.startedAt` / `run:end.endedAt`，测试见 `packages/core/test/trace.test.ts`，`npm run ci` 全绿）；**待做**：M7-2 的 `toOtelSpans` 与审计导出 → M7-1 → M7-3 → M7-6a；P5.1~P5.4 已随桌面端移出至产品侧（不再是底座保留项），不进关键路径 |
