...
- **发布阻塞项（需你在 GitHub 操作，push 前必须完成）**：2026-09-12 全量改名时 13 份 `package.json` 的 `repository` / `homepage` / `bugs` 已改指 `https://github.com/0end1/node-agent-runtime`，但**GitHub 侧仓库至今仍叫 `nodeRuntime`**（`git remote -v` 为 `https://github.com/0end1/nodeRuntime.git`）。provenance 会拿 OIDC token 里的仓库名比 `repository.url`，**不匹配即以 422 失败、12 包全部发不出去** → ① 先把 GitHub 仓库 `nodeRuntime` 改名为 `node-agent-runtime`；② 本地 `git remote set-url origin https://github.com/0end1/node-agent-runtime.git`；③ 再 push。**同时更正** `docs/m7-base-governance.md` §6 原「上条 422 风险实际不存在」的结论 —— 那次核实只比对了 12 包之间的内部一致性，未与 `git remote` 对齐
