...
| **第一批** | **M8-1 生态入口**：脚手架 `create-node-agent-runtime` **✅ 已落地（2026-09-17，第 14 个包，零运行时依赖；生成物已真跑通）** + 示例库（治理 / 续跑 / 审计三条主线）☐ + 文档站 ☐ | 底座（分发设施，原 M7-7 底座部分） | 新用户 5 分钟内跑通第一个受治理任务；脚手架生成物 `npm install && npm start` 一次通过（已在本仓库内以 workspace 软链方式实测） | 🟡 部分（脚手架 ✅ / 示例库与文档站待做） |
