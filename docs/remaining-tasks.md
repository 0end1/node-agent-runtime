...
| B 拆包批次 | B2 | 批次 2：C8 `@agent-runtime/host`（先决 C4） | crate-split-todo §3/§4 | ☐ |
...
| C 开放决策 | C1 | Session/Task 是否出 core（C8 host 做不做） | crate-architecture §8-5 | ☐ |
| C 开放决策 | C2 | sandbox/policy 独立两包 or 合成 `governance` | crate-architecture §8-3 | ☐ |
| C 开放决策 | C3 | `Artifact` 归属（草案：类型入 C1、实现并入 C3） | crate-architecture §8-4 | ☐ |
| C 开放决策 | C4 | `tool.ts`/`ToolDefinition` 契约层是否下沉 C1 | crate-architecture §8-2 | ☐ |
| D 远期 | D1 | Rust workspace 移植（对齐 codex，长期选项） | crate-architecture §7/§8-1 | ☐（远期） |
| D 远期 | D2 | `config`/`features` 配置与特性开关模块 | crate-architecture §8-7 | ☐（远期） |
...
## 2. B 拆包执行（crate-split-todo.md，动工前先落定 C1）

| 批次 | 包 | 迁移源 | 迁移测试 | 依赖 | 前置 |
|---|---|---|---|---|---|
| 1 | C6 `@agent-runtime/mcp` | `core/src/mcp/`（client/jsonrpc/registry/transport/types） | `core/test/mcp.test.ts` + fixture | C1 + core 工具契约 | core 无反向 import；收窄 index 的 mcp 导出 |
| 2 | C8 `@agent-runtime/host` | `core/src/session.ts` | `core/test/session.test.ts` | core 宿主 API + C3/C4/C5 | 决策 C1 后才动；**先搬迁后拆依赖**（上次试行回滚的教训） |
| 3 | C3 `@agent-runtime/memory` | `memory.ts` `checkpoint.ts` | `memory.test.ts` `checkpoint.test.ts` | types + core Storage trait | 决策 C3（Artifact 归属）；先拆 C8 |
| 3 | C4 `@agent-runtime/sandbox` + C5 `@agent-runtime/policy` | `sandbox.ts` + `permission.ts` | `sandbox.test.ts` `permission.test.ts`（如有） | types + core gate 调用点类型 | 与 C5 交叉引用；决策 C2（独立 or 合并） |
| 4 | facade 收窄 | `core/src/index.ts` 直出改逐包 re-export | 全量测试 | 全部包 | C6/C8/C3~C5 拆完；评估 C4 是否顺带下沉 C1 |
...
## 3. C 开放决策点（crate-architecture §8，未定不阻塞功能开发，但阻塞对应拆包）

| # | 决策 | 影响 | 现状建议 | 状态 |
|---|---|---|---|---|
| C1 | **Session/Task 是否出 core**（C8 host） | 决定 C8 做不做，及 C3~C5 能否拆干净（session.ts 是唯一宿主）；曾试行拆 C8 后回滚 | 接受 session 依赖 runtime 公开 API 的事实；如需引擎侧也用 Task，再评估拆出 task 状态机 | ☐ |
| C2 | sandbox 与 policy 独立两包 or 合成 `@agent-runtime/governance` | 决定批次 3 拆分次数 | 按 codex 推荐独立（C4/C5）；草案照此 | ☐ |
| C3 | `Artifact` 归属 | 类型入 C1；实现随 M4 并 C6 or C3 | 草案并入 C3（同为 Storage 读写） | ☐ |
| C4 | `tool.ts`/`ToolDefinition` 契约层是否下沉 C1 | TS 下 `import type` 可不拆；**转 Rust 前必须拆** | 非阻塞；facade 收窄时顺带评估 | ☐ |
...
| D2 | `config`/`features` 配置与特性开关模块（仿 codex） | 宿主层需要「配置驱动装配 / 能力开关」，deepseek-harness 有 Cordis 参考 |
