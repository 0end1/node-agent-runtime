...
| **M8 · 采纳闭环与协议接入** | 依 2026-09-17 拍板：目标客户先 (a) 建生态 + 同步铺 (c) 交付物；形态先走 ACP 路径 D、不自建壳 | 🟡 **进行中（2026-09-17）**：规范核对 ✅；**M8-2 / M8-3 代码完成**（33 例单测 + 真实子进程 E2E 9 步，**GUI Client 真机联调仍待人工**）；M8-1 生态入口未开工 | 首批：生态入口（脚手架 + 示例库 + 文档站）+ ACP 规范核对；第二批：`@node-agent-runtime/acp` + 权限桥接（+ 流式，若核对为前置）；第三批：`deploy/` 的 K8s/Helm 与升级回滚说明。详见 §3.4 |
