...
| **P2** | 工程护栏与质量门 | GitHub Actions CI（typecheck/lint/test/build）、ESLint+Prettier、覆盖率门禁、跨形态 E2E（吸收 A2）、`npm audit` 门、收敛为 `npm run ci`（吸收 A3） | 🟡 **P2.1 / P2.2 / P2.5 / P2.6 ✅**（M6-14）：`.github/workflows/ci.yml`（quality / coverage / audit 三 job）+ ESLint 9 + Prettier 基线 + `npm run ci` 总闸；**P2.7 ✅** 已随总闸接入 CI；**P2.3 ✅**（M6-16）：统计口径修正为只统计本包 + 补 `types`/`tools-basic` 测试 + 阈值定档（逐包 行≥80 / 分支≥60 / 函数≥55，全仓均值 行≥90 / 分支≥78 / 函数≥85），`npm run coverage:gate` 阻断；**P2.4 ✅**（M6-17）：`scripts/e2e/` 跨形态 E2E（CLI + Web 全流程验证通过、CI `e2e` job 接入；Desktop 默认跳过） |
