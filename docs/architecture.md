...
| **M6 · 生产级改造** | demo → 生产级：包边界收口（C3~C8 外置 + facade 收窄）、CI/质量门、可观测与安全、发布工程、分发矩阵、治理文档 | `packages/{memory,sandbox,policy,host,mcp}` | 🟡 **进行中（split 分支，2026-09-07）**：P1 包边界收口完成——9 个 workspace 包、依赖单向无环（types ← {memory,sandbox,policy} ← core ← {host,mcp,provider-openai,store-sqlite}），全仓 typecheck + `npm test` 绿；**C8 host 为破坏性变更**（`SessionManager` 改从 `@agent-runtime/host` 导入）。余 P1.6 与 P2~P6 见 `docs/m6-productionization.md` |
