...
| **M6 · 生产级改造** | demo → 可用于生产 | 🟡 进行中 | **P1 已完成（Gate 1 关闭）**：C1~C4 决策落定 + C6 mcp / C8 host / C3 memory / C4 sandbox / C5 policy 外置 + facade 收窄 + 公共 API 冻结快照（`docs/api-surface.md`）；P2~P6 待办（见 §3.1） |
...
| 批次 | 主题 | 关键项 | 状态 |
|---|---|---|---|
| **P1** | 决策冻结 + 包边界收口（**发布前置**） | 落定 C1~C4 决策 → 拆包 B1 mcp / B2 host / B3 memory·sandbox·policy / B4 facade 收窄 → 公共 API 冻结快照 | ✅ 全部完成（Gate 1 已关闭，快照见 `docs/api-surface.md`） |
