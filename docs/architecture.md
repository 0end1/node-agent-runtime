...
| **M8 · 采纳闭环与协议接入** | 依 2026-09-17 两项拍板：**目标客户 = 先 (a) 开发者个体建生态、同步铺 (c) 企业合规交付物**；**形态路径 = 先走 D（ACP 适配包），不自建壳**。线①生态与交付：脚手架 `create-node-agent-runtime` + 示例库 + 文档站、`deploy/` 补 K8s/Helm 与升级回滚说明；线②协议接入：ACP 规范核对 → `@node-agent-runtime/acp` + 权限桥接 → 流式（条件前置）→ `tools-code`（延后） | `packages/acp`（新包）、`packages/core` / `provider-openai`（流式）、示例库与脚手架、`deploy/` | 🟡 **草案已立（2026-09-17），未开工**：**ACP 规范级核对已完成**（记录见 `docs/acp-spec-review.md`）—— 传输为 stdio（换行分隔 JSON-RPC，日志走 stderr）；**`session/update` 不要求 token 级分块，故流式非前置、不阻塞 ACP 包**；`session/set_mode` 有废弃风险，桥接须同时提供 Session Config Options；排期与验收见 `docs/development-checklist.md` §3.4；决策依据 `docs/product-build-paths.md` §9；每项立项前须过 `docs/base-convergence.md` §3 三问准入 |
