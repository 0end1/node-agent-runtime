...
> 现状注记（v0.2，2026-09-05）：上表为**目标**目录结构。当前已按 `docs/crate-architecture.md` §7.1 方案 A（npm workspaces）先行落地 C1 `@node-agent-runtime/types` 与 C2 `@node-agent-runtime/core`（`packages/types`、`packages/core`），引擎代码位于 C2 的 `src/`（runtime/agent/context/session/events/tool/provider/tools/providers/store），测试随包；其余包（store-sqlite/mcp/host/desktop）仍按里程碑逐步增补。
...
| **M6 · 生产级改造** | demo → 生产级：包边界收口、审查整改（P0/P1/P2）、CI/质量门、可观测与安全、发布工程、分发矩阵、治理文档 | `packages/{types,memory,artifact,sandbox,policy,core,tools-basic,mock,host,mcp,provider-openai,store-sqlite}` | 🟡 **Gate 1~4 已关闭，P5/P6 收尾中（2026-09-09）**：Gate 1（2026-09-07）→ Gate 2（P2 质量门，M6-14~17）→ Gate 3（P3 可观测与安全，M6-18~M6-21，含 P3 评审 8 项发现全部修复，见 `docs/p3-review.md`）→ Gate 4（P4 SDK 发布工程，M6-22：MIT LICENSE、12 包去 `private` 与发布元数据、`engines >=22.13` + `.nvmrc`/`packageManager`、changesets 编排、`core`/`types` peer 边界、包体积基线）；**P5 分发部署矩阵**：P5.5 生产存储基线（schema 版本化/索引/在线备份）与 P5.6 Web 部署样例（`deploy/` + `/healthz` 探活）已完成（M6-24），P5.1~P5.4 桌面形态补齐 sidecar 构建、脚本化验收与三平台 CI（M6-25），实机签名验收待 Developer ID 证书；**P6 治理文档**：P6.1/P6.2 已完成（M6-23）。**C8 host 为破坏性变更**（`SessionManager` 改从 `@node-agent-runtime/host` 导入）。逐项状态见 `docs/m6-productionization.md` |
| **M7 · 底座收敛与治理交付** | 以 **12 包为唯一一等公民**：成本与上下文治理、可观测与合规导出、策略工程化、工具规模治理；`examples/` 与 `deploy/` 降级为验证载体，桌面端移出至产品侧 | `packages/{types,core,policy,mcp,provider-openai}` 增量 + 文档口径收敛 | 🟡 **进行中（M7-1 / M7-2 全量 / M7-3 / M7-6a / M7-6b / **M7-5 底座部分** 已交付，2026-09-15）**：立项口径与边界见 `docs/base-convergence.md`（只对底座立项）；候选与首批建议见 `docs/product-direction.md` §5（首批 M7-1/2/3/6a）；**执行清单见 `docs/m7-base-governance.md`**（2026-09-10 立：各包落点 / API 变更分级 / 验收用例 / 顺序）；**已交付**：批次 A（版本 bump 到 0.3.0，未发布、未打 tag）与 **M7-2 的 traceId 贯穿**（19 个事件统一 `traceId?` + `emit()` 与 `redact()` 同点收口 + `Logger.child` + `run:start.startedAt` / `run:end.endedAt`，测试见 `packages/core/test/trace.test.ts`，`npm run ci` 全绿）；**已交付**：M7-6b（MCP 只读资源 `resources/list` / `resources/read` + `searchTools`，资源物化为只读工具、只认已声明 URI、敏感度保守取 `network-read`，见 §5.3 追注）；**已交付**：**M7-5 底座部分**（Agent 配方编译 `compileAgent()` + 配方快照 `instructionsHash`，2026-09-15 同日交付，执行清单见 `docs/m7-base-governance.md` §9；并发调度按 `base-convergence.md` §6 需先论证，不进本批）—— 首批四项 + 6b + M7-5 底座部分全部落地，**无待做项**；P5.1~P5.4 已随桌面端移出至产品侧（不再是底座保留项），不进关键路径 |
...
| v1.3 (workspace 化) | 2026-09-05 | 代码按 `docs/crate-architecture.md` §7.1 收敛为 npm workspaces：C1 `@node-agent-runtime/types` / C2 `@node-agent-runtime/core`（C2 re-export C1 保持公共 API 不变）；§10 目标结构下包形态落地注记；详见 crate-architecture.md v0.2 修订 |
...
| v1.8 (M6-P1) | 2026-09-07 | **包边界收口（M6 P1，split 分支）**：C3 `@node-agent-runtime/memory`（memory + artifact）、C4 `@node-agent-runtime/sandbox`、C5 `@node-agent-runtime/policy`、C6 `@node-agent-runtime/mcp`、C8 `@node-agent-runtime/host`（`session.ts`）依次外置为独立包，`core` 收窄为 facade（`export *` 转发 memory/sandbox/policy；mcp 与 host 因方向为「子包 → core」不反向 re-export）；共享契约下沉 C1：`Storage`/`DocDomain`/`StreamDomain`、`Artifact`/`ArtifactKind`/`ArtifactInput`、工具契约（`ToolDefinition`/`AnyTool`/`ToolKind`/`ToolMeta`/`ToolExecutionContext`）、事件契约（`RuntimeEvent` 等 + 新增 `EventEmitter<E>`）。**破坏性变更**：`SessionManager` 改从 `@node-agent-runtime/host` 导入。§10 现状注记更新为 9 个 workspace 包结构；§11 新增 M6 行。详见 `docs/crate-architecture.md` v0.5~v0.10 与 `docs/crate-split-todo.md` |
| v1.9 (M6 审查整改) | 2026-09-08 | **P1 审查整改闭环（M6-9~M6-11，split 分支）**：按 `docs/p1-review.md` P0/P1/P2 整改——① 演示资产外置（`@node-agent-runtime/tools-basic` / `@node-agent-runtime/mock`）；② `artifact.ts` 自 C3 拆出为独立包 `@node-agent-runtime/artifact`（一包一职责）；③ 工具分类 `classifyToolName`/`toolKind` 下沉 C1（sandbox re-export 保兼容，mcp 去除对 sandbox 依赖）；④ `checkpoint.ts` 以 `ToolSurface` 契约解耦 `Agent` 后归位 C3 memory，core 由 **1804 → 1005 行**、收窄为引擎 + facade；⑤ 事件总线可注入（`AgentRuntimeOptions.events` / `SessionManagerOptions.events`，host 统一 `this.events`）；⑥ 公共 API 面按 **12 包**重新冻结（`docs/api-surface.md`），快照复核脚本化（M6-11：`scripts/check-api-surface.ts` + 基线 + `npm run check:api`）。§10 现状注记 / §11 M6 行同步更新。详见 `docs/m6-productionization.md`、`docs/api-surface.md` §13 |
| v1.10 (M6-P2~P6) | 2026-09-09 | **生产级改造收尾（M6-14~M6-25）**：P2 质量门（`npm run ci` 一键收口 typecheck → lint → test → coverage:gate → `check:api` → 体积门禁，CI 编排 + 跨形态 E2E）；P3 可观测与安全（结构化日志 + 稳定错误码、事件/日志脱敏、审批审计与 `always` 白名单持久化、成本与速率上限、Web 鉴权与防跨站、MCP 供应链防护（含重定向 SSRF 与子进程 env 最小化）、默认安全策略包、`loadConfig` 配置分层，评审与修复见 `docs/p3-review.md`）；P4 SDK 发布工程（MIT LICENSE、12 包发布元数据、Node `>=22.13` 与 `.nvmrc`/`packageManager`、changesets + `release.yml`（`npm publish --provenance`）、`core`/`types` peer 边界、发布 d.ts 自包含（`ProcessEnv`）、包体积基线）；P5 分发部署（store-sqlite `SCHEMA_VERSION` 版本化迁移 + session/task/run 索引 + `VACUUM INTO` 在线备份、`deploy/` 部署样例与 `/healthz` 探活、桌面 sidecar 构建脚本与三平台构建矩阵）；P6 治理文档（`CONTRIBUTING.md`、`SECURITY.md`、README 生产用法与 badges）。§11 M6 行同步 |
| v1.11 (底座收敛) | 2026-09-10 | **项目口径收敛（判定事实源：`docs/base-convergence.md`）**：确立「**底座 = `packages/*` 12 包**」为唯一一等公民；`examples/cli.ts`、`examples/web/`、`deploy/`、`scripts/e2e/`、`scripts/smoke-web.mjs` 降级为**验证载体（只验证底座，不演进产品）**；`examples/desktop-tauri/`、`.github/workflows/desktop.yml`、`scripts/verify-desktop.mjs`、`scripts/e2e/desktop.mjs` 及 P5.1~P5.4 标注**冻结**（保留代码与恢复条件，不进关键路径）；垂直应用 / 消费级桌面 / Rust 移植 / 多 Agent 协同明确不立项。§1 增补「运行时分层 ≠ 投入分层」口径注记（Desktop 不再是顶层投入对象）；§11 新增 M7 行（只对底座立项）。配套同步：`docs/product-direction.md`（控制台升级路径撤销、M7 候选重归类）、`README.md`、`docs/development-checklist.md`、`docs/remaining-tasks.md` |
| v1.12 (桌面端移出) | 2026-09-10 | **底座收敛取消桌面端，方向移入产品侧**（判定事实源：`docs/base-convergence.md` §2.3）：`examples/desktop-tauri/`、`.github/workflows/desktop.yml`、`scripts/verify-desktop.mjs`、`scripts/e2e/desktop.mjs` 与 **P5.1~P5.4 整体移出底座范围**——底座不再投入，也不再判定其立项与否；原 §2.4「消费级桌面助手 → 不立项」条目撤销。§1 口径注记与 §11 M7 行同步更新（投入分层 = 底座 → 验证载体，桌面端不属任何底座分类）。配套同步：`docs/m6-productionization.md` §5（「挂起决定」回填为「移出决定」、Gate 5 退出标准修订、P5.1~P5.4 状态改 ⏸）、`docs/product-direction.md`（§0/§3/§4/§6/§7/§8 承接桌面形态候选与决策项）、`docs/remaining-tasks.md`、`docs/development-checklist.md`、`examples/desktop-tauri/README.md` 与 `.github/workflows/desktop.yml` 顶部注记（冻结 → 移出至产品侧） |
