...
- **补 M8 全批 changeset（发布链路断点）**：M7 每个特性都带独立 changeset（`m7-1-cost-context.md` 等），而 **M8-1 ~ M8-5 与 P3 三项一个都没有** —— 直接合 `main` 时 changesets 的 Version Packages PR 不会 bump 版本，OIDC 发布根本不会触发（代码完成但发不出去）。本次按特性补 6 个 changeset（M8-1 脚手架 / M8-2 ACP 包 / M8-3 权限桥接 / M8-4 流式 / P3 审计开关 / P3 修复），`changeset status` 确认 **12 个发布包全部 minor → 0.5.0**（新增 `acp` 与 `create-node-agent-runtime` 两包 + 大量 additive API）。两点记录：① `mock` / `tools-basic` 已转 `private`，**不再列入 changeset**（0.4.2 那次的 changeset 还含它们，是历史状态）；② `create-node-agent-runtime` **不带 scope、不在 `fixed` 组内**，本次因自身 changeset 而同为 minor，但若将来某次发布只改 scope 包，它的版本会独立漂移 —— 需要时把它加进 `.changeset/config.json` 的 `fixed`。M8-5 的 Helm 与示例库 / 文档站不是 npm 包，无需 changeset。
- **M8 文档回填（发版状态与 changeset 补齐记录）**：`docs/architecture.md` §11 M8 行补「发布准备 ✅ 已通（12 包 minor → 0.5.0）」，§13 新增 **v1.24** 记录 M8 全批漏带 changeset 的补齐过程与三点记录（`mock` / `tools-basic` 已 `private` 不再列入；`create-node-agent-runtime` 不带 scope、不在 `fixed` 组内；Helm 与示例库 / 文档站非 npm 包无需 changeset）；`docs/development-checklist.md` §0 M8 行同步发版状态、§3.4 新增「发布」行。另记一笔教训：`CONTRIBUTING.md` §6 检查清单本就含「用户可见改动 → 已加 changeset + `CHANGELOG.md` 条目」，M8 系**执行遗漏而非流程缺失**，故不改流程文档。
