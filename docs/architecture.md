...
| **M8 · 采纳闭环与协议接入** | 依 2026-09-17 两项拍板：**目标客户 = 先 (a) 开发者个体建生态、同步铺 (c) 企业合规交付物**；**形态路径 = 先走 D（ACP 适配包），不自建壳**。线①生态与交付：脚手架 `create-node-agent-runtime` + 示例库 + 文档站、`deploy/` 补 K8s/Helm 与升级回滚说明；线②协议接入：ACP 规范核对 → `@node-agent-runtime/acp` + 权限桥接 → 流式（条件前置）→ `tools-code`（延后） | `packages/acp`（新包）、`packages/core` / `provider-openai`（流式）、示例库与脚手架、`deploy/` | ✅ **代码全部完成（2026-09-18）**，**待人工验收与发布 0.5.0**：规范核对已完成（记录见 `docs/acp-spec-review.md`）—— 传输为 stdio（换行分隔 JSON-RPC，日志走 stderr）；**`session/update` 不要求 token 级分块，故流式非前置**。其中 **M8-2 已代码完成**：新包 `packages/acp`（第 13 个包），stdio 传输经真实子进程冒烟验证；**M8-3 已代码完成**：`session/request_permission` ↔ `PermissionManager` 桥接（客户端不实现该方法时降级为拒绝，不再空等审批超时）+ `session/set_mode` ↔ `SandboxMode` 并同时提供 `session/set_config_option`（官方明示 set_mode 将被移除，故双轨），共 33 例测试通过；**新增协议级 E2E**（`npm run e2e -- --only=acp`，真实 agent 子进程 + 自写协议级客户端，9 步覆盖审批往返 / 模式切换 / 飞行中取消 / stdout 纯净），**GUI 客户端（Zed / DeepChat）真机联调仍待人工**（不在 CI 内），执行清单见 `docs/m8-acp-adapter.md`；**M8-1 脚手架 `create-node-agent-runtime` 已落地**（第 14 个包，零运行时依赖，模板编译进 `dist`；生成物已在本仓库内以 workspace 软链方式实测 `npm start` 通过）+ **示例库已落地**（`examples/cookbook/`，治理 / 续跑 / 审计三条主线，三个示例实测可跑）+ **文档站已落地**（`docs/` 即站点源，VitePress，`npm run docs:dev`）—— **M8-1 三项全部完成**；**M8-4 流式已代码完成**（2026-09-18：`ModelProvider` 增**可选** `chatStream()` + `message:delta` 事件 + `provider-openai` 的 SSE 解析；须显式 `stream` 开启且默认 false，ACP 侧默认 true）；**M8-5 `deploy/` K8s/Helm 与升级回滚已落地**（2026-09-18：`deploy/helm/node-agent-runtime/`，复用同一 Dockerfile；`deploy/README.md` §7/§8）；排期与验收见 `docs/development-checklist.md` §3.4；决策依据 `docs/product-build-paths.md` §9；每项立项前须过 `docs/base-convergence.md` §3 三问准入；**发布准备 ✅ 已通（2026-09-18）**：M8-1 ~ M8-5 与 P3 三项此前**整批漏带 changeset**（M7 为每个特性各带一份），已按特性补齐 6 份，`npx changeset status` 确认 12 个发布包**全部 minor → 0.5.0**，发版链路已通（详见 §13 v1.24） |
...
| v1.21 (ACP 协议级 E2E 与真机入口) | 2026-09-17 | **M8-2 / M8-3 真机前验证自动化**：新增 `scripts/e2e/acp.mjs`（`npm run e2e -- --only=acp`，已并入默认 `npm run e2e` 与 `scripts/e2e/run-all.mjs` 的 targets）。取舍：GUI Client 真机联调无法进 CI，故把「除客户端呈现之外的一切」自动化 —— spawn **真实 agent 子进程**（`packages/acp/test/fixtures/stdio-agent-governed.ts`，带 write 类工具以触发审批），用**自写的协议级客户端**（复用包导出的 `LineDecoder` / `encodeMessage` / `parseMessage` 自行解帧，并作为 server 方应答 `session/request_permission`）经真实 stdio 走完 9 步：握手 / `session/new` 双选择面同值 / 审批往返（pending → allow_once → completed，参数脱敏）/ `session/load` 回放 / `set_config_option` 切 read-only（回 `current_mode_update`）/ 只读下写工具被策略**直接拒绝、不再弹审批**（证明模式改变的是策略而非 UI）/ 飞行中 `session/cancel` → `cancelled`（非 error）/ 关闭后拒绝服务 / stdout 无协议外输出。另新增真机入口 `examples/acp-agent.ts`（`npm run demo:acp`：带 `OPENAI_API_KEY` 接真实模型、否则退 `MockProvider`；诊断走 stderr 以保证 stdout 纯净）。分工：单测走内存传输验逻辑，E2E 走真实进程验**帧与语义一起成立**，两者不互相替代。**GUI 客户端（Zed / DeepChat）联调仍待人工**。配套同步：`docs/m8-acp-adapter.md` §4、`docs/development-checklist.md`（§0 M8 行回填 + §3.4 状态）、§11 M8 行、`CHANGELOG.md` |
| v1.22 (M8-4 流式) | 2026-09-18 | **M8-4 流式代码完成** —— 给底座补齐「模型边想边说」，**全部 additive、默认与 M8-4 之前逐字一致**。① **契约**：`types` 新增 `MessageDeltaEvent`（`message:delta`，含 `runId` / `step` / `delta` / `index` / 可选 `traceId`），`ModelProvider` 增**可选** `chatStream(request, onDelta)` —— 未实现即「不支持流式」，老 provider 零改动；② **引擎**：`RunOptions.stream` / `AgentRuntimeOptions.stream` 显式开启（**默认 false**）才走流式，`chatStream` 的调用收口在 `callModel` 单一处；③ **`provider-openai`**：新增 `src/sse.ts`（跨 chunk 半帧重组、工具调用参数按 `index` 累积、`[DONE]` 终止）；④ **ACP**：`AcpAgentOptions.stream`（**默认 true** —— ACP 客户端本就把回复当流渲染）把增量推为 `agent_message_chunk`，已流式流过的消息在 `model:response` 时**不再整段补发**（否则客户端看到两遍文本）。三条关键取舍：**回退只在「还没吐出任何增量之前」发生**（已出块再失败直接走 run 的错误路径 —— 服务端已接收并计费，重跑会重复扣费，且消费者会先收到半截增量再收到一整段重复文本）；**流式是体验增强不是能力前提**（provider 未实现则静默降级，不是错误）；**脱敏逐块进行**，故跨块边界的敏感串不被识别，完整文本的安全保证仍由整段脱敏的 `model:response` 承担。测试 core 106 / provider-openai 9 / acp 37 全通过；`docs/api-surface.md` 快照同步（types 69→70、core 78→79）。配套同步：§11 M8 行、`docs/development-checklist.md` §3.4、`docs/m8-acp-adapter.md`、`CHANGELOG.md` |
| v1.23 (M8-5 K8s/Helm) | 2026-09-18 | **M8-5 `deploy/` 补 K8s/Helm 形态与升级回滚说明**（对应 (c) 企业合规交付物）：新增 `deploy/helm/node-agent-runtime/`（Chart + values + 模板，**复用同一个 Dockerfile** —— 多阶段、非 root、`/healthz` 探活，只换编排层）。五条关键取舍：① **`replicaCount: 1` + `strategy: Recreate`**（SQLite 是**本机文件**存储，多副本会各写各的库致会话与审批审计串台，且 `ReadWriteOnce` 卷无法被新旧 Pod 同时挂载，RollingUpdate 会卡在 Multi-Attach）；② **ConfigMap/Secret 的 `checksum` 注解**（少了它改完配置 Pod 不重启，「我以为生效了」是最难查的一类事故）；③ **密钥只走 `existingSecret` 或 `--set-string`**（values 会进 Git，也会出现在 `helm get values` 的输出里）；④ **`readOnlyRootFilesystem: true` + `/tmp` 内存盘、SA token 不挂载**（控制台不调 K8s API）；⑤ **PVC 带 `helm.sh/resource-policy: keep`**（删 release 不删数据，否则「回滚」根本无从谈起）。`deploy/README.md` 新增 §7 K8s/Helm 与 §8 升级回滚：**代码可回滚，数据未必** —— schema 只前进（`PRAGMA user_version`），回滚到旧镜像而库已升级时旧程序**拒绝启动而非静默破坏数据**，故固定顺序「先备份 → 再升级；先回滚代码 → 必要时才恢复数据」。配套同步：§11 M8 行、`docs/development-checklist.md` §3.4、`deploy/README.md`、`CHANGELOG.md` |
| v1.24 (M8 changeset 补齐) | 2026-09-18 | **补 M8 全批 changeset，打通发布链路**（本条无功能代码改动）：M7 每个特性都带独立 changeset（`m7-1-cost-context.md` / `m7-3-policy-engineering.md` 等），而 **M8-1 ~ M8-5 与 P3 三项一个都没有** —— 直接合 `main` 时 changesets 的 Version Packages PR 不会 bump 版本，OIDC 发布不会触发，即「代码完成但发不出去」。本次按特性补 6 份（`m8-1-scaffold` / `m8-2-acp` / `m8-3-acp-permission` / `m8-4-streaming` / `p3-audit-policy-allows` / `p3-fixes`），`npx changeset status` 确认 **12 个发布包全部 minor → 0.5.0**（新增 `acp` 与 `create-node-agent-runtime` 两包 + 大量 additive API）。三点记录：① `mock` / `tools-basic` 已转 `private`，**不再列入 changeset**（0.4.2 那次的 changeset 仍含二者，属历史状态）；② `create-node-agent-runtime` **不带 scope、不在 `fixed` 组内**，本次因自身 changeset 同为 minor，但若将来某次发布只改 scope 包则其版本会独立漂移 —— 需要时加进 `.changeset/config.json` 的 `fixed`；③ M8-5 的 Helm 与示例库 / 文档站不是 npm 包，无需 changeset。**防复发**：`CONTRIBUTING.md` §6 检查清单本就含「用户可见改动 → 已加 changeset + `CHANGELOG.md` 条目」，M8 系执行时遗漏而非流程缺失，故不改流程文档、仅记录教训。配套同步：§11 M8 行、`docs/development-checklist.md` §0 / §3.4、`CHANGELOG.md` |
